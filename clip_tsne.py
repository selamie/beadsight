...
from clip_pretraining import modified_resnet18, get_beadsight_clip, ClipProjectionHead, ClipDataset
...
"""
This file is used to visualize the latent vectors of the images and beadsight 
data using t-SNE. Its useful to see how well the pre-training works.
"""

def plot_tsne(all_image_vectors:np.ndarray, beadsight_vectors:np.ndarray, timestamps):
    """
    Plots the t-SNE visualization of the latent vectors of the images and beadsight data.
    all_image_vectors: np.ndarray of shape (n_cam, episode_len, 512). The latent vectors of the images.
    beadsight_vectors: np.ndarray of shape (episode_len, 512). The latent vectors of the beadsight data.
    ...
    # concatenate all the latent vectors, including the beadsight vectors
    all_latent_vectors = np.concatenate([all_image_vectors[i] for i in range(n_cam)] + [beadsight_vectors], axis=0)

    # fit the t-SNE model to the latent vectors
    embedded = tsne.fit_transform(all_latent_vectors)

    # get the image and beadsight TSNE embeddings
    image_embedings = [embedded[i*episode_len:(i+1)*episode_len] for i in range(n_cam)]
    beadsight_embedded = embedded[n_cam*episode_len:]
    ...
    plt.scatter(beadsight_embedded[:, 0], beadsight_embedded[:, 1], c=timestamps/len(timestamps), cmap='viridis', marker='x', s=50, alpha=0.25, label='Beadsight Vectors')
...
if __name__ == "__main__":
    ...
    # clip model paths
    image_encoder_path = "/media/selamg/DATA/beadsight/data/clipmodels/epoch_1499_vision_encoder.pth"
    image_projection_head_path = "/media/selamg/DATA/beadsight/data/clipmodels/epoch_1499_vision_projection.pth"
    beadsight_encoder_path = "/media/selamg/DATA/beadsight/data/clipmodels/epoch_1499_beadsight_encoder.pth"
    beadsight_projection_head_path = "/media/selamg/DATA/beadsight/data/clipmodels/epoch_1499_beadsight_projection.pth"
    ...
    if use_act:
        ...
        beadsight_encoder = act.model.backbones[1][0]
        ...
        # create the beadsight encoder and projection head
        beadsight_encoder = get_beadsight_clip().to(device)
        if encoder_pretrained:
            beadsight_encoder.load_state_dict(torch.load(beadsight_encoder_path, map_location=device))
    ...
    if use_projection_head:
        ...
        beadsight_projection_head = ClipProjectionHead(512, 3).to(device)
        # beadsight_projection_head = ClipProjectionHead(512).to(device)
        if  projection_head_pretrained:
            beadsight_projection_head.load_state_dict(torch.load(beadsight_projection_head_path, map_location=device))

    vision_encoder.eval()
    beadsight_encoder.eval()
    if use_projection_head:
        vision_projection_head.eval()
        beadsight_projection_head.eval()
    ...
    for episode_idx in episode_idxs:
        ...
        with torch.no_grad():
            for t in tqdm(range(episode_len)):
                ...
                # get the encoding and clip projection for the beadsight data
                encoding = beadsight_encoder(beadsight.to(device).unsqueeze(0))
                if use_projection_head:
                    beadsight_vector = beadsight_projection_head(encoding, position.to(device).unsqueeze(0))
                    beadsight_vectors[t, :] = beadsight_vector.detach().cpu().numpy().squeeze()
                else:
                    encoding = F.adaptive_avg_pool2d(encoding, (1, 1))
                    beadsight_encodings[t, :] = encoding.detach().cpu().numpy().squeeze()
        ...
        if not use_projection_head:
            ...
            beadsight_vectors = beadsight_encodings

        # plot the similarity matrix for the latent vectors (intra-run similarity)
        plot_run_similarity(beadsight_vectors, 'beadsight')
        ...
        plot_tsne(all_image_vectors, beadsight_vectors, timestamps)
