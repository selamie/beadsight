...
class NormalizeDiffusionActionQpos:
    ...
    def __call__(self, qpos, action):
        ...
        if np.min(action) < -1 or np.max(action) > 1:
            print(f"outside bounds for action min or max, got min, max action: {np.min(action),np.max(action)}")
            
        if np.min(qpos) < -1 or np.max(qpos) > 1:
            print(f"outside bounds for qpos min or max, got min max qpos: {np.min(qpos),np.max(qpos)}")
